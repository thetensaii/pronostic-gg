...
export default class GetLeaderboardController {
  async handle({ auth, request, response }: HttpContext) {
    ...
    if (payload.params.league_code === 'main') {
      const maybeCompetition = await CompetitionModel.findBy(
        'slug',
        payload.params.competition_slug
      )
      if (!maybeCompetition) {
    ...
    const endedMatches = await competition
      .related('matches')
      .query()
      .whereNotNull('score_a')
      .andWhereNotNull('score_b')
    const membersPointsDto = await Promise.all(
      members.map((m) => this.computeMemberPoints(m, endedMatches))
    )
  ...
  private async computeMemberPoints(
    member: UserModel,
    endedMatches: MatchModel[]
  ): Promise<Member> {
    const endedMatchesIds = endedMatches.map((m) => m.id)
    const forecasts = await ForecastModel.query()
      .where('user_id', member.id)
      .andWhereIn('match_id', endedMatchesIds)
    ...
    const points = endedMatches.reduce<number>((acc, currentMatch) => {
      ...
      if (
        forecast.teamAScore === currentMatch.scoreA &&
        forecast.teamBScore === currentMatch.scoreB
      )
        return acc + 1
      return acc
    }, 0)
  ...
  private computeRank(memberPoints: Member[]) {
    ...
    for (const [i, sortedMember] of sortedMembers.entries()) {
      if (sortedMember.points < currentPoints) {
        currentRank = i + 1
        currentPoints = sortedMember.points
      }
      rankedMembers.push({ ...sortedMember, rank: currentRank })
    }
