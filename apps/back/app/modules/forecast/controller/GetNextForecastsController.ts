...
export default class GetNextForecastsController {
  async handle({ auth, request, response }: HttpContext) {
    ...
    const matches = await competition
      .related('matches')
      .query()
      .whereNull('score_a')
      .andWhereNull('score_b')
    const matchesIds = matches.map((m) => m.id)
    const userForecasts = await ForecastModel.query()
      .where('user_id', user.id)
      .andWhereIn('match_id', matchesIds)

    return await Promise.all(
      matches.map(async (match) => this.convertMatchToForecastDto(match, userForecasts))
    )
  }

  private async convertMatchToForecastDto(
    match: MatchModel,
