...
export default class GetForecastsResultsController {
  async handle({ auth, request, response }: HttpContext) {
    ...
    const userForecasts = await ForecastModel.query()
      .where('user_id', user.id)
      .andWhereIn('match_id', endedMatchesIds)

    return await Promise.all(
      endedMatches.map(async (match) => this.convertMatchToForecastResultDto(match, userForecasts))
    )
  }

  private async convertMatchToForecastResultDto(
    match: MatchModel,
    userForecasts: ForecastModel[]
    ...
    const forecastScore: [number, number] | null =
      // eslint-disable-next-line eqeqeq
      forecast?.teamAScore == null || forecast?.teamBScore == null
        ? null
        : [forecast.teamAScore, forecast.teamBScore]
  ...
  private isForecastGood(
    matchScore: [number, number],
    forecastScore: [number, number] | null
  ): boolean {
