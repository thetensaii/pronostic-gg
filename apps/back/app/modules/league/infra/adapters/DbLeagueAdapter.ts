...
export class DbLeagueAdapter {
  async toDomain(dbLeague: LeagueModel, dbMembers: DbMember[]): Promise<League> {
    return new League({
      id: dbLeague.id,
      name: dbLeague.name,
      code: dbLeague.code,
      ...
      members: dbMembers.map(
        (dbMember) => new Member({ id: dbMember.user_id, joinedAt: dbMember.created_at })
      ),
