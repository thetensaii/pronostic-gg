...
export default class ForecastProvider {
  ...
  register() {
    this.app.container.singleton(UserService, () => this.app.container.make(DbUserService))
    this.app.container.singleton(CompetitionService, () =>
      this.app.container.make(DbCompetitionService)
    )
    this.app.container.singleton(LeagueRepository, () =>
      this.app.container.make(DbLeagueRepository)
    )

    this.app.container.singleton(CreateLeagueUseCase, () => this.app.container.make(CreateLeague))
    this.app.container.singleton(JoinLeagueUseCase, () => this.app.container.make(JoinLeague))
  }
}
